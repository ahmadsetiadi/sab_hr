...
type
  TFrmELeave = class(TForm)
    ...
    MASTERtotalleave: TcxGridDBBandedColumn;
    MASTERtakenleave: TcxGridDBBandedColumn;
    MASTERavailableleave: TcxGridDBBandedColumn;
    MASTERColumn1: TcxGridDBBandedColumn;
    MASTERColumn2: TcxGridDBBandedColumn;
    MASTERColumn3: TcxGridDBBandedColumn;
    ApplicationEvents1: TApplicationEvents;
    MASTERColumn4: TcxGridDBBandedColumn;
    MASTERColumn5: TcxGridDBBandedColumn;
...
procedure TFrmELeave.VisibleALLGridDetail(tipe:boolean=False);
begin
...
procedure TFrmELeave.BtnCloseClick(Sender: TObject);
begin
  Close;
end;
procedure TFrmELeave.cekAllEditModule(idm:integer);
var
  md : string;
  qk : tzquery;
begin
...
procedure TFrmELeave.isiALLTableKolom;
var
  idm : integer;
  qk : tzquery;
begin
...
procedure TFrmELeave.isiDataMenuPrint;
var
  x : integer;
  idm, mn, tipe : string;
  qm : tzquery;
...
procedure TFrmELeave.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmELeave.FormCreate(Sender: TObject);
begin
  isdebug := true;
...
procedure TFrmELeave.SettingQuery;
var
  idd, idm : integer;
  qd  : tzquery;
  namagrid : string;
begin
  ...
  setColumnWidth(txtid.Text, q_master, master, False, 'm_employee2');
...
procedure TFrmELeave.SettingGridDetail;
var
  idd, idm : integer;
  qd  : tzquery;
  namagrid : string;
...
procedure TFrmELeave.ReloadClick;
var
  idm : integer;
begin
  idm               := strtoint(txtid.Text);
...
procedure TFrmELeave.SettingFont;
var
  x, fs : integer;
  fn : string;
begin
...
procedure TFrmELeave.ValidasiControl;
var
  mn : string;
begin
...
procedure TFrmELeave.ShowDateTemplate(Tipe:Boolean=True);
begin
...
procedure TFrmELeave.BtnStartdateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmELeave.BtnStartdatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  startdate := BtnStartdate.Date;
end;
procedure TFrmELeave.BtnEnddateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmELeave.BtnEnddatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  enddate := BtnEnddate.Date;
end;
procedure TFrmELeave.MenuItem1Click(Sender: TObject);
begin
  //first date to now
...
procedure TFrmELeave.OneMonth1Click(Sender: TObject);
begin
  //one month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmELeave.NextMonth1Click(Sender: TObject);
begin
  //next month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmELeave.PrevMonth1Click(Sender: TObject);
begin
  //prev month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmELeave.OneYear1Click(Sender: TObject);
begin
  //one year
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmELeave.OneDay1Click(Sender: TObject);
begin
  //oneday
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmELeave.oday1Click(Sender: TObject);
begin
  //today
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmELeave.actStartdateExecute(Sender: TObject);
begin
  BtnStartdate.SetFocus;
end;
procedure TFrmELeave.actEnddateExecute(Sender: TObject);
begin
  BtnEnddate.SetFocus;
end;
...
//grid
function TFrmELeave.getFocusQuery : TZQuery;
begin
...
function TFrmELeave.getFocusTableName : String;
var
  x : integer;
  idtag : integer;
begin
...
function TFrmELeave.getFocusGrid : TcxGridDBBandedTableView;
var
  x : integer;
begin
...
function TFrmELeave.getFocusGridView : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
function TFrmELeave.getActiveModuleName : string;
var
  id1 : string;
begin
...
procedure TFrmELeave.ApplicationEvents1Exception(Sender: TObject; E: Exception);
begin
  MsgError(e.Message);
  isNowEdit[strtoint(txtid.Text)]    := False;
end;

procedure TFrmELeave.AutoActiveQuery;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmELeave.AutoRefreshQuery;
var
  x : integer;
  qk : TZquery;
begin
  ...
  qk := createquery;
  qk.Query('select kolom_id from s_kolom where tablename=''m_employee2'' and tipekolom=''lookup'' order by no');
  qk.First;
  while not qk.Eof do
  begin
...
function TFrmELeave.getTabByName(nama:string) : TcxTabSheet;
begin
  nama := lowercase(nama);
...
function TFrmELeave.getCxGridByName(nama:string) : TcxGrid;
begin
  nama := lowercase(nama);
...
function TFrmELeave.getGridByName(nama:string) : TcxGridDBBandedTableView;
begin
  nama := lowercase(nama);
...
function TFrmELeave.getQueryByName(nama:string) : TZQuery;
begin
  nama := lowercase(nama);
...
procedure TFrmELeave.actLookupExecute(Sender: TObject);
var
  Q : tzquery;
  s, sql : string;
  idk : integer;
...
procedure TFrmELeave.actAddExecute(Sender: TObject);
begin
...
procedure TFrmELeave.actcancelExecute(Sender: TObject);
begin
  getQueryByName(getFocusGridView.Name).Cancel;
end;
procedure TFrmELeave.actDeleteExecute(Sender: TObject);
begin
...
procedure TFrmELeave.actEditExecute(Sender: TObject);
begin
...
procedure TFrmELeave.actRefreshExecute(Sender: TObject);
begin
   //getQueryByName(getFocusGridView.Name).Refresh;
   q_master.Refresh;
end;
procedure TFrmELeave.actSaveExecute(Sender: TObject);
begin
  if isnowedit[getQueryByName(getFocusGridView.Name).Tag]  then getQueryByName(getFocusGridView.Name).Post;
end;
procedure TFrmELeave.actTabDetail1Execute(Sender: TObject);
begin
  exit;
...
procedure TFrmELeave.actTabDetail2Execute(Sender: TObject);
begin
  exit;
  if PageDetail.Visible = true then
  begin
...
procedure TFrmELeave.actTabMasterExecute(Sender: TObject);
begin
...
//popup menu
procedure TFrmELeave.ppExportClick(Sender: TObject);
begin
  if Confirm('Export Excel ?') = 'YES' then  ExportGrid('Excel', getCxGridByName(getfocusgridview.Name) );
end;
procedure TFrmELeave.PopupMenuPopup(Sender: TObject);
var
  mn : string;
begin
...
procedure TFrmELeave.ppFreezeClick(Sender: TObject);
var
  caption : string;
  x : integer;
begin
...
procedure TFrmELeave.ppGroupDefaultClick(Sender: TObject);
begin
  GroupDateFormatClick(getFocusGridView, 'default');
end;
procedure TFrmELeave.ppGroupMonthClick(Sender: TObject);
begin
  GroupDateFormatClick(getFocusGridView, 'month');
end;
procedure TFrmELeave.ppGroupYearClick(Sender: TObject);
begin
  GroupDateFormatClick(getFocusGridView, 'year');
end;
procedure TFrmELeave.RelativeToday1Click(Sender: TObject);
begin
  GroupDateFormatClick(getFocusGridView, 'relative');
end;
procedure TFrmELeave.ppFindClick(Sender: TObject);
begin
...
procedure TFrmELeave.ppShowALLClick(Sender: TObject);
begin
  //dbg(getQueryByName(getFocusGridView.Name).Name);
...
//isnowedit master
procedure TFrmELeave.Q_MASTERNewRecord(DataSet: TDataSet);
begin
  NewRecordQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmELeave.Q_MASTERBeforeEdit(DataSet: TDataSet);
begin
  BeforeEditQ(dataset.Tag, Dataset.Name);
end;
procedure TFrmELeave.Q_MASTERBeforeDelete(DataSet: TDataSet);
begin
  BeforeDeleteQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmELeave.Q_MASTERAfterDelete(DataSet: TDataSet);
begin
  AfterDeleteQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmELeave.Q_MASTERBeforePost(DataSet: TDataSet);
begin
  BeforePostQ(DataSet.Tag, Dataset.Name);
end;
procedure TFrmELeave.Q_MASTERAfterPost(DataSet: TDataSet);
begin
  AfterPostQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmELeave.Q_MASTERAfterRefresh(DataSet: TDataSet);
begin
  AutoRefreshQuery;
end;
procedure TFrmELeave.Q_MASTERAfterCancel(DataSet: TDataSet);
begin
...
//procedure query
procedure TFrmELeave.AfterCancelQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
...
procedure TFrmELeave.AfterDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmELeave.AfterPostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmELeave.BeforeDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
...
procedure TFrmELeave.BeforeEditQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
procedure TFrmELeave.BeforePostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
...
procedure TFrmELeave.NewRecordQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
//master
procedure TFrmELeave.newRecordMaster;
begin
  AutoRefreshQuery;
end;
procedure TFrmELeave.beforePostMaster;
begin
  //
end;
procedure TFrmELeave.afterPostMaster;
begin
  q_master.Refresh;
end;
procedure TFrmELeave.beforeEditMaster;
begin
  //
end;
procedure TFrmELeave.beforeDeleteMaster;
begin
  //
end;
procedure TFrmELeave.afterDeleteMaster;
begin
  //
end;
procedure TFrmELeave.afterCancelMaster;
begin
  //
end;
//end master
...
//mutation q_detail1
procedure TFrmELeave.newRecordTotal;
begin
  //
end;
procedure TFrmELeave.beforePostTotal;
begin
  //
end;
procedure TFrmELeave.afterPostTotal;
begin
  //
end;
procedure TFrmELeave.beforeEditTotal;
begin
  //
end;
procedure TFrmELeave.beforeDeleteTotal;
begin
  //
end;
procedure TFrmELeave.afterDeleteTotal;
begin
  //
end;
procedure TFrmELeave.afterCancelTotal;
begin
  //
end;
//end mutation
...
//Relatives q_detail2
procedure TFrmELeave.newRecordTaken;
begin
  //
end;
procedure TFrmELeave.beforePostTaken;
begin
  //
end;
procedure TFrmELeave.afterPostTaken;
begin
  //
end;
procedure TFrmELeave.beforeEditTaken;
begin
  //
end;
procedure TFrmELeave.beforeDeleteTaken;
begin
  //
end;
procedure TFrmELeave.afterDeleteTaken;
begin
  //
end;
procedure TFrmELeave.afterCancelTaken;
begin
  //
end;
//end Relatives
...
procedure TFrmELeave.N4IsiMasterCuti1Click(Sender: TObject);
begin
  ShowProgressbar;
  q_master.DisableControls;
  q_master.First;
