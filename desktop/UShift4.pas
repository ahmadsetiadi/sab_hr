...
procedure TFrmShift4.BtnCloseClick(Sender: TObject);
begin
  Close;
end;
procedure TFrmShift4.cekAllEditModule(idm:integer);
var
  md : string;
  qk : tzquery;
begin
...
procedure TFrmShift4.isiALLTableKolom;
var
  idm : integer;
  qk : tzquery;
begin
...
procedure TFrmShift4.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmShift4.FormCreate(Sender: TObject);
begin
...
procedure TFrmShift4.SettingQuery;
var
  idd, idm : integer;
  qd  : tzquery;
  namagrid : string;
...
procedure TFrmShift4.ReloadClick;
var
  idm : integer;
begin
  idm               := strtoint(txtid.Text);
...
procedure TFrmShift4.SettingFont;
var
  x, fs : integer;
  fn : string;
begin
...
procedure TFrmShift4.ValidasiControl;
var
  mn : string;
begin
...
procedure TFrmShift4.ShowDateTemplate(Tipe:Boolean=True);
begin
...
procedure TFrmShift4.BtnStartdateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmShift4.BtnStartdatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  startdate := BtnStartdate.Date;
end;
procedure TFrmShift4.BtnEnddateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmShift4.BtnEnddatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  enddate := BtnEnddate.Date;
end;

procedure TFrmShift4.MenuItem1Click(Sender: TObject);
begin
  //first date to now
...
procedure TFrmShift4.OneMonth1Click(Sender: TObject);
begin
  //one month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmShift4.NextMonth1Click(Sender: TObject);
begin
  //next month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmShift4.PrevMonth1Click(Sender: TObject);
begin
  //prev month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmShift4.OneYear1Click(Sender: TObject);
begin
  //one year
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmShift4.PopupMenuPopup(Sender: TObject);
var
  mn : string;
begin
...
procedure TFrmShift4.OneDay1Click(Sender: TObject);
begin
  //oneday
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmShift4.oday1Click(Sender: TObject);
begin
  //today
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmShift4.actStartdateExecute(Sender: TObject);
begin
  BtnStartdate.SetFocus;
end;
procedure TFrmShift4.actEnddateExecute(Sender: TObject);
begin
  BtnEnddate.SetFocus;
end;
...
//grid
function TFrmShift4.getFocusQuery : TZQuery;
begin
  if lowercase(getFocusGrid.Name) = 'master'  then result := Q_MASTER;
end;
function TFrmShift4.getFocusTableName : String;
var
  x : integer;
  idtag : integer;
begin
...
function TFrmShift4.getFocusGrid : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
function TFrmShift4.getFocusGridView : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmShift4.AutoActiveQuery;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmShift4.AutoRefreshQuery;
var
  x : integer;
  qk : TZquery;
begin
...
function TFrmShift4.getTabByName(nama:string) : TcxTabSheet;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := TabMaster;
end;

function TFrmShift4.getGridByName(nama:string) : TcxGridDBBandedTableView;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := master;
end;
function TFrmShift4.getQueryByName(nama:string) : TZQuery;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := q_master;
end;
procedure TFrmShift4.actLookupExecute(Sender: TObject);
var
  Q : tzquery;
  s, sql : string;
  idk : integer;
...
procedure TFrmShift4.actAddExecute(Sender: TObject);
begin
  if cekValidasi('`insert`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Inserting = False then exit;
  q_master.Append;
end;
procedure TFrmShift4.actcancelExecute(Sender: TObject);
begin
  q_master.Cancel;
end;
procedure TFrmShift4.actDeleteExecute(Sender: TObject);
begin
...
procedure TFrmShift4.actEditExecute(Sender: TObject);
begin
  if cekValidasi('`edit`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Editing = False then exit;
  q_master.Edit;
end;
procedure TFrmShift4.actRefreshExecute(Sender: TObject);
begin
  q_master.Refresh;
end;
procedure TFrmShift4.actSaveExecute(Sender: TObject);
begin
  if isnowedit[strtoint(txtid.text)]  then q_master.Post;
end;
procedure TFrmShift4.actTabDetail1Execute(Sender: TObject);
begin
  //
end;
procedure TFrmShift4.actTabMasterExecute(Sender: TObject);
begin
...
//popup menu
procedure TFrmShift4.ppExportClick(Sender: TObject);
begin
  if Confirm('Export to Excel ?') = 'YES' then  ExportGrid('Excel', grid_master);
end;
procedure TFrmShift4.ppFreezeClick(Sender: TObject);
var
  caption : string;
  x : integer;
begin
...
procedure TFrmShift4.ppGroupDefaultClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'default');
end;
procedure TFrmShift4.ppGroupMonthClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'month');
end;
procedure TFrmShift4.ppGroupYearClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'year');
end;
procedure TFrmShift4.RelativeToday1Click(Sender: TObject);
begin
  GroupDateFormatClick(master, 'relative');
end;
procedure TFrmShift4.ppFindClick(Sender: TObject);
begin
  FindClick(q_master, Master, Tablename[strtoint(txtid.Text)]);
end;
procedure TFrmShift4.ppShowALLClick(Sender: TObject);
begin
...
//isnowedit master
procedure TFrmShift4.Q_MASTERNewRecord(DataSet: TDataSet);
begin
  NewRecordQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmShift4.Q_MASTERBeforeEdit(DataSet: TDataSet);
begin
  BeforeEditQ(dataset.Tag, Dataset.Name);
end;
procedure TFrmShift4.Q_MASTERBeforeDelete(DataSet: TDataSet);
begin
  BeforeDeleteQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmShift4.Q_MASTERAfterDelete(DataSet: TDataSet);
begin
  AfterDeleteQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmShift4.Q_MASTERBeforePost(DataSet: TDataSet);
begin
  BeforePostQ(DataSet.Tag, Dataset.Name);
end;
procedure TFrmShift4.Q_MASTERAfterPost(DataSet: TDataSet);
begin
  AfterPostQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmShift4.Q_MASTERAfterRefresh(DataSet: TDataSet);
begin
  AutoRefreshQuery;
end;
procedure TFrmShift4.Q_MASTERAfterCancel(DataSet: TDataSet);
begin
...
//procedure query
procedure TFrmShift4.AfterCancelQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
...
procedure TFrmShift4.AfterDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmShift4.AfterPostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmShift4.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
  MsgError(e.Message);
...
procedure TFrmShift4.BeforeDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  if nama='master'  then beforeDeleteMaster;
end;
procedure TFrmShift4.BeforeEditQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
procedure TFrmShift4.BeforePostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
...
procedure TFrmShift4.NewRecordQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
//master
procedure TFrmShift4.newRecordMaster;
begin
  AutoRefreshQuery;
...
procedure TFrmShift4.beforePostMaster;
//var
//  q1 : tzquery;
begin
...
//  end;
  ...
  q_master.setField('tdate', Firstday(q_master.getFieldDateTime('tdate')));
end;
procedure TFrmShift4.afterPostMaster;
begin
  q_master.Refresh;
end;
procedure TFrmShift4.beforeEditMaster;
begin
  //
end;
procedure TFrmShift4.beforeDeleteMaster;
begin
...
//  end;
  ...
  DeleteAllDataDetail(txtID.Text, q_master.getFieldString('tshift_id'));
end;
procedure TFrmShift4.afterDeleteMaster;
begin
  //
end;
procedure TFrmShift4.afterCancelMaster;
begin
  //
end;
//end master

function TFrmShift4.CheckPayrollExists(sdate, edate:tdatetime): boolean;
begin
...
procedure TFrmShift4.MASTERColumn1PropertiesPopup(Sender: TObject);
var
  Qe : tzquery;
begin
  Qe := CreateQuery;
  Qe.Query('select employee_id,name,nip,company_id,'+
           'department_id,position_id,employeestatus_id,harikerja '+es+
