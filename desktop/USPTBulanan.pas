...
procedure TFrmSPTBulanan.BtnCloseClick(Sender: TObject);
begin
  Close;
end;
procedure TFrmSPTBulanan.cekAllEditModule(idm:integer);
var
  md : string;
  qk : tzquery;
begin
...
procedure TFrmSPTBulanan.isiALLTableKolom;
var
  idm : integer;
  qk : tzquery;
begin
...
procedure TFrmSPTBulanan.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmSPTBulanan.FormCreate(Sender: TObject);
begin
  isdebug            := true;
  txtID.Text         := IDModule;
...
procedure TFrmSPTBulanan.CreateSPTBulanan;
var
  qp, qs : tzquery;
begin
  qp := createquery;
  ...
  qp.Query('select * from t_payroll where tdate>='''+date2sql(startdate)+''' and '+es+
            'tdate<='''+date2sql(enddate)+''' and roundtaxableincome > 0 order by nip');
  if qp.RecordCount > 0 then ShowProgressbar;
  qp.First;
  while not qp.Eof do
  begin
    SetProgressbarDefault(qp);
    qs.Query('select * from t_spt1 where '+gets('employee_id', qp)+' and '+es+
             'month(tdate)= '''+qp.getMonthSQL('tdate')+''' and '+es+
             'year(tdate)= '''+qp.getYearSQL('tdate')+''' ');
    if qs.RecordCount = 0 then
    begin
      qs.Append;
      isiKolomUser(true, qs);
      qs.setField('masapajak',  strtoint(qp.getMonthSQL('tdate')) );
      qs.setField('tahunpajak', strtoint(qp.getYearSQL('tdate')) );
      qs.setField('pembetulan', '0');
      qs.setField('kodepajak', '21-100-01');
      qs.setField('kodenegara', '');
    end else
    begin
      qs.Edit;
    ...
    qs.copyData(qp);
    qs.setField('tax21_monthly', qp.getFieldDouble('thrbonustax')+qp.getFieldDouble('tax21_monthly')+qp.getFieldDouble('tax21_irregular')  );
    qs.Post;
    qp.Next;
  end;
  qp.Free;
...
procedure TFrmSPTBulanan.SettingQuery;
var
  idd, idm : integer;
  qd  : tzquery;
  namagrid : string;
...
procedure TFrmSPTBulanan.ReloadClick;
var
  idm : integer;
begin
  idm               := strtoint(txtid.Text);
...
procedure TFrmSPTBulanan.SettingFont;
var
  x, fs : integer;
  fn : string;
begin
...
procedure TFrmSPTBulanan.ValidasiControl;
var
  mn : string;
begin
...
procedure TFrmSPTBulanan.PopupMenuPopup(Sender: TObject);
var
  mn : string;
begin
...
procedure TFrmSPTBulanan.ShowDateTemplate(Tipe:Boolean=True);
begin
...
procedure TFrmSPTBulanan.BtnStartdateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmSPTBulanan.BtnStartdatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  startdate := BtnStartdate.Date;
end;
procedure TFrmSPTBulanan.BtnEnddateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmSPTBulanan.BtnEnddatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  enddate := BtnEnddate.Date;
end;
procedure TFrmSPTBulanan.MenuItem1Click(Sender: TObject);
begin
  //first date to now
...
procedure TFrmSPTBulanan.OneMonth1Click(Sender: TObject);
begin
  //one month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmSPTBulanan.NextMonth1Click(Sender: TObject);
begin
  //next month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmSPTBulanan.PrevMonth1Click(Sender: TObject);
begin
  //prev month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmSPTBulanan.OneYear1Click(Sender: TObject);
begin
  //one year
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmSPTBulanan.OneDay1Click(Sender: TObject);
begin
  //oneday
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmSPTBulanan.oday1Click(Sender: TObject);
begin
  //today
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmSPTBulanan.actStartdateExecute(Sender: TObject);
begin
  BtnStartdate.SetFocus;
end;
procedure TFrmSPTBulanan.actEnddateExecute(Sender: TObject);
begin
  BtnEnddate.SetFocus;
end;
...
//grid
function TFrmSPTBulanan.getFocusQuery : TZQuery;
begin
  if lowercase(getFocusGrid.Name) = 'master'  then result := Q_MASTER;
end;
function TFrmSPTBulanan.getFocusTableName : String;
var
  x : integer;
  idtag : integer;
begin
...
function TFrmSPTBulanan.getFocusGrid : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
function TFrmSPTBulanan.getFocusGridView : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmSPTBulanan.AutoActiveQuery;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmSPTBulanan.AutoRefreshQuery;
var
  x : integer;
  qk : TZquery;
begin
...
function TFrmSPTBulanan.getTabByName(nama:string) : TcxTabSheet;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := TabMaster;
end;
procedure TFrmSPTBulanan.ImportFromExcel1Click(Sender: TObject);
var
  tn : string;
  qc, qk, qi : tzquery;
  idm : integer;
...
function TFrmSPTBulanan.getGridByName(nama:string) : TcxGridDBBandedTableView;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := master;
end;
function TFrmSPTBulanan.getQueryByName(nama:string) : TZQuery;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := q_master;
end;
procedure TFrmSPTBulanan.actLookupExecute(Sender: TObject);
var
  Q : tzquery;
  s, sql : string;
  idk : integer;
...
procedure TFrmSPTBulanan.actAddExecute(Sender: TObject);
begin
  if cekValidasi('`insert`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Inserting = False then exit;
  q_master.Append;
end;
procedure TFrmSPTBulanan.actcancelExecute(Sender: TObject);
begin
  q_master.Cancel;
end;
procedure TFrmSPTBulanan.actDeleteExecute(Sender: TObject);
begin
...
procedure TFrmSPTBulanan.actEditExecute(Sender: TObject);
begin
  if cekValidasi('`edit`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Editing = False then exit;
  q_master.Edit;
end;
procedure TFrmSPTBulanan.actRefreshExecute(Sender: TObject);
begin
  q_master.Refresh;
end;
procedure TFrmSPTBulanan.actSaveExecute(Sender: TObject);
begin
  if isnowedit[strtoint(txtid.text)]  then q_master.Post;
end;
procedure TFrmSPTBulanan.actTabDetail1Execute(Sender: TObject);
begin
  //
end;
procedure TFrmSPTBulanan.actTabMasterExecute(Sender: TObject);
begin
...
//popup menu
procedure TFrmSPTBulanan.ppExportClick(Sender: TObject);
begin
  if Confirm('Export to Excel ?') = 'YES' then  ExportGrid('Excel', grid_master);
end;
procedure TFrmSPTBulanan.ppFreezeClick(Sender: TObject);
var
  caption : string;
  x : integer;
begin
...
procedure TFrmSPTBulanan.ppGroupDefaultClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'default');
end;
procedure TFrmSPTBulanan.ppGroupMonthClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'month');
end;
procedure TFrmSPTBulanan.ppGroupYearClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'year');
end;
procedure TFrmSPTBulanan.RelativeToday1Click(Sender: TObject);
begin
  GroupDateFormatClick(master, 'relative');
end;
procedure TFrmSPTBulanan.ppFindClick(Sender: TObject);
begin
  FindClick(q_master, Master, Tablename[strtoint(txtid.Text)]);
end;
procedure TFrmSPTBulanan.ppShowALLClick(Sender: TObject);
begin
...
//isnowedit master
procedure TFrmSPTBulanan.Q_MASTERNewRecord(DataSet: TDataSet);
begin
  NewRecordQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmSPTBulanan.Q_MASTERBeforeEdit(DataSet: TDataSet);
begin
  BeforeEditQ(dataset.Tag, Dataset.Name);
end;
procedure TFrmSPTBulanan.Q_MASTERBeforeDelete(DataSet: TDataSet);
begin
  BeforeDeleteQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmSPTBulanan.Q_MASTERAfterDelete(DataSet: TDataSet);
begin
  AfterDeleteQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmSPTBulanan.Q_MASTERBeforePost(DataSet: TDataSet);
begin
  BeforePostQ(DataSet.Tag, Dataset.Name);
end;
procedure TFrmSPTBulanan.Q_MASTERAfterPost(DataSet: TDataSet);
begin
  AfterPostQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmSPTBulanan.Q_MASTERAfterRefresh(DataSet: TDataSet);
begin
  AutoRefreshQuery;
end;
procedure TFrmSPTBulanan.Q_MASTERAfterCancel(DataSet: TDataSet);
begin
...
//procedure query
procedure TFrmSPTBulanan.AfterCancelQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
...
procedure TFrmSPTBulanan.AfterDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmSPTBulanan.AfterPostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmSPTBulanan.BeforeDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  if nama='master'  then beforeDeleteMaster;
end;
procedure TFrmSPTBulanan.BeforeEditQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
procedure TFrmSPTBulanan.BeforePostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
...
procedure TFrmSPTBulanan.NewRecordQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
//master
procedure TFrmSPTBulanan.newRecordMaster;
begin
  AutoRefreshQuery;
  q_master.setField('tdate', serverdate);
end;
procedure TFrmSPTBulanan.afterPostMaster;
begin
  q_master.Refresh;
end;
procedure TFrmSPTBulanan.beforePostMaster;
begin
  //
end;
procedure TFrmSPTBulanan.beforeEditMaster;
begin
  //
end;
procedure TFrmSPTBulanan.beforeDeleteMaster;
begin
  //
end;
procedure TFrmSPTBulanan.afterDeleteMaster;
begin
  //
end;
procedure TFrmSPTBulanan.afterCancelMaster;
begin
  //
end;
//end master

procedure TFrmSPTBulanan.MASTERemployee_idPropertiesPopup(Sender: TObject);
begin
  LookupADEmployee_id;
end;
procedure TFrmSPTBulanan.LookupADEmployee_id;
var
  Q : TZQuery;
...
procedure TFrmSPTBulanan.N2ImportBuktiPotong1721A11Click(Sender: TObject);
var
  nama, fn, npwppemotong, s, npwp: string;
  ts : tstringlist;
begin
  ...
  begin
    nama := getQValueString('select name from m_employee where '+gets('employee_id', q_master)+' ');
    SetProgressbar('Employee : '+q_master.getFieldString('nip')+
                   ' '+nama+' '+
                   ', '+inttostr(q_master.RecNo)+'/'+inttostr(q_master.recordcount),
                   (q_master.recno * 100) div q_master.recordcount);
    npwp := q_master.getCSV('npwpemployee');
    npwp := replace(npwp, '.', '');
    npwp := replace(npwp, '-', '');
    s := '';
    s := s + q_master.getCSV('masapajak')   + ';' + q_master.getCSV('tahunpajak')    + ';';
    s := s + q_master.getCSV('pembetulan')  + ';' + npwp                             + ';';
    s := s + nama                           + ';' + q_master.getCSV('kodepajak')     + ';';
    s := s + q_master.getCSV('totalgross')  + ';' + q_master.getCSV('tax21_monthly') + ';';
    s := s + q_master.getCSV('kodenegara');
    ts.Add(s);
    q_master.Next;
  end;
