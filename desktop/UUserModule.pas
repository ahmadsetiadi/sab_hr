...
type
  TFrmUserModule = class(TForm)
    ...
    MASTERinsert: TcxGridDBBandedColumn;
    MASTERedit: TcxGridDBBandedColumn;
    MASTERdelete: TcxGridDBBandedColumn;
    MASTERexport: TcxGridDBBandedColumn;
    MASTERmenu: TcxGridDBBandedColumn;
    MASTERprint: TcxGridDBBandedColumn;
    MASTERdisable_menu: TcxGridDBBandedColumn;
    MASTERdisable_print: TcxGridDBBandedColumn;
    MASTERdisable_detail: TcxGridDBBandedColumn;
    MASTERhide_column: TcxGridDBBandedColumn;
    MASTERreadonly_column: TcxGridDBBandedColumn;
    ...
    MASTERhide_column2: TcxGridDBBandedColumn;
    MASTERreadonly_column2: TcxGridDBBandedColumn;
    ApplicationEvents1: TApplicationEvents;
    MASTERimport: TcxGridDBBandedColumn;
    ...
    procedure N1SetMultiModule1Click(Sender: TObject);
    procedure MASTERdisable_menuPropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure MASTERdisable_printPropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure MASTERdisable_detailPropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure MASTERhide_column2PropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
    procedure MASTERreadonly_column2PropertiesButtonClick(Sender: TObject;
      AButtonIndex: Integer);
...
procedure TFrmUserModule.BtnCloseClick(Sender: TObject);
begin
  Close;
end;
procedure TFrmUserModule.cekAllEditModule(idm:integer);
var
  md : string;
  qk : tzquery;
begin
...
procedure TFrmUserModule.isiALLTableKolom;
var
  idm : integer;
  qk : tzquery;
begin
...
procedure TFrmUserModule.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmUserModule.FormCreate(Sender: TObject);
begin
  txtID.Text         := IDModule;
...
procedure TFrmUserModule.SettingQuery;
var
  idd, idm : integer;
  qd  : tzquery;
  namagrid : string;
...
procedure TFrmUserModule.ReloadClick;
var
  idm : integer;
begin
  idm               := strtoint(txtid.Text);
...
procedure TFrmUserModule.SettingFont;
var
  x, fs : integer;
  fn : string;
begin
...
procedure TFrmUserModule.ValidasiControl;
var
  mn : string;
begin
...
procedure TFrmUserModule.ShowDateTemplate(Tipe:Boolean=True);
begin
...
procedure TFrmUserModule.BtnStartdateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmUserModule.BtnStartdatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  startdate := BtnStartdate.Date;
end;
procedure TFrmUserModule.BtnEnddateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmUserModule.BtnEnddatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  enddate := BtnEnddate.Date;
end;
procedure TFrmUserModule.MenuItem1Click(Sender: TObject);
begin
  //first date to now
...
procedure TFrmUserModule.OneMonth1Click(Sender: TObject);
begin
  //one month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmUserModule.NextMonth1Click(Sender: TObject);
begin
  //next month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmUserModule.PrevMonth1Click(Sender: TObject);
begin
  //prev month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmUserModule.OneYear1Click(Sender: TObject);
begin
  //one year
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmUserModule.OneDay1Click(Sender: TObject);
begin
  //oneday
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmUserModule.oday1Click(Sender: TObject);
begin
  //today
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmUserModule.actStartdateExecute(Sender: TObject);
begin
  BtnStartdate.SetFocus;
end;
procedure TFrmUserModule.actEnddateExecute(Sender: TObject);
begin
  BtnEnddate.SetFocus;
end;
...
//grid
function TFrmUserModule.getFocusQuery : TZQuery;
begin
  if lowercase(getFocusGrid.Name) = 'master'  then result := Q_MASTER;
end;
function TFrmUserModule.getFocusTableName : String;
var
  x : integer;
  idtag : integer;
begin
...
function TFrmUserModule.getFocusGrid : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
function TFrmUserModule.getFocusGridView : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmUserModule.AutoActiveQuery;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmUserModule.AutoRefreshQuery;
var
  x : integer;
  qk : TZquery;
begin
...
function TFrmUserModule.getTabByName(nama:string) : TcxTabSheet;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := TabMaster;
end;
procedure TFrmUserModule.ImportFromExcel1Click(Sender: TObject);
var
  tn : string;
  qc, qk, qi : tzquery;
  idm : integer;
...
function TFrmUserModule.getGridByName(nama:string) : TcxGridDBBandedTableView;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := master;
end;
function TFrmUserModule.getQueryByName(nama:string) : TZQuery;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := q_master;
end;
procedure TFrmUserModule.actLookupExecute(Sender: TObject);
var
  Q : tzquery;
  s, sql : string;
  idk : integer;
...
procedure TFrmUserModule.actAddExecute(Sender: TObject);
begin
  if cekValidasi('`insert`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Inserting = False then exit;
  q_master.Append;
end;
procedure TFrmUserModule.actcancelExecute(Sender: TObject);
begin
  q_master.Cancel;
end;
procedure TFrmUserModule.actDeleteExecute(Sender: TObject);
begin
...
procedure TFrmUserModule.actEditExecute(Sender: TObject);
begin
  if cekValidasi('`edit`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Editing = False then exit;
  q_master.Edit;
end;
procedure TFrmUserModule.actRefreshExecute(Sender: TObject);
begin
  q_master.Refresh;
end;
procedure TFrmUserModule.actSaveExecute(Sender: TObject);
begin
  if isnowedit[strtoint(txtid.text)]  then q_master.Post;
end;
procedure TFrmUserModule.actTabDetail1Execute(Sender: TObject);
begin
  //
end;
procedure TFrmUserModule.actTabMasterExecute(Sender: TObject);
begin
...
//popup menu
procedure TFrmUserModule.ppExportClick(Sender: TObject);
begin
  if Confirm('Export to Excel ?') = 'YES' then  ExportGrid('Excel', grid_master);
end;
procedure TFrmUserModule.ppFreezeClick(Sender: TObject);
var
  caption : string;
  x : integer;
begin
...
procedure TFrmUserModule.ppGroupDefaultClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'default');
end;
procedure TFrmUserModule.ppGroupMonthClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'month');
end;
procedure TFrmUserModule.ppGroupYearClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'year');
end;
procedure TFrmUserModule.RelativeToday1Click(Sender: TObject);
begin
  GroupDateFormatClick(master, 'relative');
end;
procedure TFrmUserModule.ppFindClick(Sender: TObject);
begin
  FindClick(q_master, Master, Tablename[strtoint(txtid.Text)]);
end;
procedure TFrmUserModule.ppShowALLClick(Sender: TObject);
begin
...
//isnowedit master
procedure TFrmUserModule.Q_MASTERNewRecord(DataSet: TDataSet);
begin
  NewRecordQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmUserModule.Q_MASTERBeforeEdit(DataSet: TDataSet);
begin
  BeforeEditQ(dataset.Tag, Dataset.Name);
end;
procedure TFrmUserModule.Q_MASTERBeforeDelete(DataSet: TDataSet);
begin
  BeforeDeleteQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmUserModule.Q_MASTERAfterDelete(DataSet: TDataSet);
begin
  AfterDeleteQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmUserModule.Q_MASTERBeforePost(DataSet: TDataSet);
begin
  BeforePostQ(DataSet.Tag, Dataset.Name);
end;
procedure TFrmUserModule.Q_MASTERAfterPost(DataSet: TDataSet);
begin
  AfterPostQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmUserModule.Q_MASTERAfterRefresh(DataSet: TDataSet);
begin
  AutoRefreshQuery;
end;
procedure TFrmUserModule.Q_MASTERAfterCancel(DataSet: TDataSet);
begin
...
//procedure query
procedure TFrmUserModule.AfterCancelQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
...
procedure TFrmUserModule.AfterDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmUserModule.AfterPostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmUserModule.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
  MsgError(e.Message);
...
procedure TFrmUserModule.BeforeDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  if nama='master'  then beforeDeleteMaster;
end;
procedure TFrmUserModule.BeforeEditQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
procedure TFrmUserModule.BeforePostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
...
procedure TFrmUserModule.NewRecordQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
//master
procedure TFrmUserModule.newRecordMaster;
begin
  AutoRefreshQuery;
end;
procedure TFrmUserModule.beforePostMaster;
begin
  q_master.setField('groupname', getQValueString('select groupname from s_usergroup where '+gets('id_usergroup', q_master)+' '));
  q_master.setField('modulename', getQValueString('select name from s_module where '+gets('id_module', q_master)+' '));
end;
procedure TFrmUserModule.afterPostMaster;
begin
  q_master.Refresh;
end;
procedure TFrmUserModule.beforeEditMaster;
begin
  //
end;
procedure TFrmUserModule.beforeDeleteMaster;
begin
  //
end;
procedure TFrmUserModule.afterDeleteMaster;
begin
  //
end;
procedure TFrmUserModule.afterCancelMaster;
begin
  //
end;
//end master

procedure TFrmUserModule.LookupADEmployee_id;
var
  Q : TZQuery;
...
procedure TFrmUserModule.N1SetMultiModule1Click(Sender: TObject);
var
  qu : tzquery;
begin
  qu := createquery;
  qu.Query('select id_usergroup, groupname from s_usergroup order by groupname');
  if LookupQuery('Choose User Group', qu, 800, False) = False then
  begin
    qu.Free;
    exit;
  end;
  FrmTreeView.txtTipe.Text    := 'usermodule';
  FrmTreeView.txtID_User.Text := qu.getFieldString('id_usergroup');
  FrmTreeView.isiTreeView;
  FrmTreeView.ShowModal;
  qu.Free;
  q_master.Refresh;
end;
procedure TFrmUserModule.MASTERdisable_menuPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  q_master.Cancel;
  FrmTreeView.txtKolom1.Text  := 'disable_menu';
  FrmTreeView.txtTipe.Text    := 'menu';
  FrmTreeView.txtID_User.Text := q_master.getFieldString('id_usergroup');
  FrmTreeView.txtID_Module.Text := q_master.getFieldString('id_module');
  FrmTreeView.isiTreeView2('Disable Menu', 'select id_menu, name from s_menu where '+gets('id_module',q_master)+' and tipe=''menu'' and $syarat order by name', q_master.getFieldString('disable_menu'));
  FrmTreeView.ShowModal;
  q_master.Refresh;
end;
procedure TFrmUserModule.MASTERdisable_printPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  q_master.Cancel;
  FrmTreeView.txtKolom1.Text  := 'disable_print';
  FrmTreeView.txtTipe.Text    := 'menu';
  FrmTreeView.txtID_User.Text := q_master.getFieldString('id_usergroup');
  FrmTreeView.txtID_Module.Text := q_master.getFieldString('id_module');
  FrmTreeView.isiTreeView2('Disable Print', 'select id_menu, name from s_menu where '+gets('id_module',q_master)+' and tipe=''print'' and $syarat order by name', q_master.getFieldString('disable_print'));
  FrmTreeView.ShowModal;
  q_master.Refresh;
end;
procedure TFrmUserModule.MASTERdisable_detailPropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  q_master.Cancel;
  FrmTreeView.txtKolom1.Text  := 'disable_detail';
  FrmTreeView.txtTipe.Text    := 'menu';
  FrmTreeView.txtID_User.Text := q_master.getFieldString('id_usergroup');
  FrmTreeView.txtID_Module.Text := q_master.getFieldString('id_module');
  FrmTreeView.isiTreeView2('Disable Detail', 'select id_module, name from s_module where parentid='''+q_master.getFieldString('id_module')+''' and $syarat and isdetail=1 order by name', q_master.getFieldString('disable_detail'));
  FrmTreeView.ShowModal;
  q_master.Refresh;
end;
procedure TFrmUserModule.MASTERhide_column2PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  q_master.Cancel;
  FrmTreeView.txtKolom1.Text  := 'hide_column';
  FrmTreeView.txtKolom2.Text  := 'hide_column2';
  FrmTreeView.txtTipe.Text    := 'column';
  FrmTreeView.txtID_User.Text := q_master.getFieldString('id_usergroup');
  FrmTreeView.txtID_Module.Text := q_master.getFieldString('id_module');
  FrmTreeView.isiTreeView2('Hide Column', 'select kolom_id, captionname as name from s_kolom where '+gets('id_module', q_master)+' and $syarat and _visible=1 order by captionname', q_master.getFieldString('hide_column2'));
  FrmTreeView.ShowModal;
  q_master.Refresh;
end;

procedure TFrmUserModule.MASTERreadonly_column2PropertiesButtonClick(
  Sender: TObject; AButtonIndex: Integer);
begin
  q_master.Cancel;
  FrmTreeView.txtKolom1.Text  := 'readonly_column';
  FrmTreeView.txtKolom2.Text  := 'readonly_column2';
  FrmTreeView.txtTipe.Text    := 'column';
  FrmTreeView.txtID_User.Text := q_master.getFieldString('id_usergroup');
  FrmTreeView.txtID_Module.Text := q_master.getFieldString('id_module');
  FrmTreeView.isiTreeView2('Readonly Column', 'select kolom_id, captionname as name from s_kolom where '+gets('id_module', q_master)+' and $syarat and _visible=1 and _readonly=0 order by captionname', q_master.getFieldString('readonly_column2'));
  FrmTreeView.ShowModal;
  q_master.Refresh;
end;
