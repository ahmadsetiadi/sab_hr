...
{$R *.dfm}
uses unit1, UMenuLogin, UImport, UAd;
//form close, form create
procedure TFrmAd2.BtnCloseClick(Sender: TObject);
begin
  Close;
end;
procedure TFrmAd2.cekAllEditModule(idm:integer);
var
  md : string;
  qk : tzquery;
begin
...
procedure TFrmAd2.isiALLTableKolom;
var
  idm : integer;
  qk : tzquery;
begin
...
procedure TFrmAd2.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFrmAd2.FormCreate(Sender: TObject);
begin
  isDebug            := True;
  txtID.Text         := IDModule;
...
procedure TFrmAd2.SettingQuery;
var
  idd, idm : integer;
  qd  : tzquery;
  namagrid : string;
...
procedure TFrmAd2.ValidasiControl;
var
  mn : string;
begin
...
procedure TFrmAd2.ShowDateTemplate(Tipe:Boolean=True);
begin
...
procedure TFrmAd2.BtnStartdateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmAd2.BtnStartdatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  startdate := BtnStartdate.Date;
end;
procedure TFrmAd2.BtnEnddateKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ReloadClick;
end;

procedure TFrmAd2.BtnEnddatePropertiesChange(Sender: TObject);
begin
  if DisableChangeDate then exit;
  enddate := BtnEnddate.Date;
end;
procedure TFrmAd2.MenuItem1Click(Sender: TObject);
begin
  //first date to now
...
procedure TFrmAd2.OneMonth1Click(Sender: TObject);
begin
  //one month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmAd2.NextMonth1Click(Sender: TObject);
begin
  //next month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmAd2.PrevMonth1Click(Sender: TObject);
begin
  //prev month
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmAd2.OneYear1Click(Sender: TObject);
begin
  //one year
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmAd2.PopupMenuPopup(Sender: TObject);
var
  mn : string;
begin
...
procedure TFrmAd2.OneDay1Click(Sender: TObject);
begin
  //oneday
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmAd2.oday1Click(Sender: TObject);
begin
  //today
  if BtnDateTemplate.Visible = False then Exit;
  DisableChangeDate := true;
...
procedure TFrmAd2.actStartdateExecute(Sender: TObject);
begin
  BtnStartdate.SetFocus;
end;
procedure TFrmAd2.actEnddateExecute(Sender: TObject);
begin
  BtnEnddate.SetFocus;
end;
...
//grid
function TFrmAd2.getFocusQuery : TZQuery;
begin
  if lowercase(getFocusGrid.Name) = 'master'  then result := Q_MASTER;
end;
function TFrmAd2.getFocusTableName : String;
var
  x : integer;
  idtag : integer;
begin
...
function TFrmAd2.getFocusGrid : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
function TFrmAd2.getFocusGridView : TcxGridDBBandedTableView;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmAd2.AutoActiveQuery;
var
  x : integer;
begin
  for x := 0 to ComponentCount - 1 do
...
procedure TFrmAd2.AutoRefreshQuery;
var
  x : integer;
  qk : TZquery;
begin
...
function TFrmAd2.getTabByName(nama:string) : TcxTabSheet;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := TabMaster;
end;
procedure TFrmAd2.IsiSKolom(idm, tn:String);
var
  q1, q2 : tzquery;
begin
  q1 := createquery;
...
procedure TFrmAd2.ImportFromExcel1Click(Sender: TObject);
var
  AdaSh : Boolean;
...
Label
  LabelError, LabelOK;
begin
  z  := TZConnection.Create(FrmAd2);
  qe := createquery;
  tsS:= tstringlist.Create;
  idm:= inttostr(getQueryByName(getFocusGridView.Name).Tag);
  tn := getTable(idm);
  IsiSKolom(idm, tn);
...
procedure TFrmAd2.ImportExcel(idm, tn, sheet : string; qe:tzquery);
var
  qc, qm, qk, ql : tzquery;
  tsk, tse : tstringlist;
  x : integer;
...
procedure TFrmAd2.afterPost(tn:string; qm : tzquery);
begin
  tn := lowercase(tn);
...
procedure TFrmAd2.beforePost(tn:string;qm : tzquery);
var
  qem : tzquery;
begin
  qem:= createquery;
...
function TFrmAd2.getGridByName(nama:string) : TcxGridDBBandedTableView;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := master;
end;
function TFrmAd2.getQueryByName(nama:string) : TZQuery;
begin
  nama := lowercase(nama);
  if nama = 'master'  then result := q_master;
end;
procedure TFrmAd2.actLookupExecute(Sender: TObject);
var
  Q : tzquery;
  s, sql : string;
  idk : integer;
...
procedure TFrmAd2.actAddExecute(Sender: TObject);
begin
  if cekValidasi('`insert`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Inserting = False then exit;
  q_master.Append;
end;
procedure TFrmAd2.actcancelExecute(Sender: TObject);
begin
  q_master.Cancel;
end;
procedure TFrmAd2.actDeleteExecute(Sender: TObject);
begin
...
procedure TFrmAd2.actEditExecute(Sender: TObject);
begin
  if cekValidasi('`edit`', txtmodulename.Text) = false then exit;
  if master.OptionsData.Editing = False then exit;
  q_master.Edit;
end;
procedure TFrmAd2.actRefreshExecute(Sender: TObject);
begin
  q_master.Refresh;
end;
procedure TFrmAd2.actSaveExecute(Sender: TObject);
begin
  if isnowedit[strtoint(txtid.text)]  then q_master.Post;
end;
procedure TFrmAd2.actTabDetail1Execute(Sender: TObject);
begin
  //
end;
procedure TFrmAd2.actTabMasterExecute(Sender: TObject);
begin
...
//popup menu
procedure TFrmAd2.ppExportClick(Sender: TObject);
begin
  if Confirm('Export to Excel ?') = 'YES' then  ExportGrid('Excel', grid_master);
end;
procedure TFrmAd2.ppFreezeClick(Sender: TObject);
var
  caption : string;
  x : integer;
begin
...
procedure TFrmAd2.ppGroupDefaultClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'default');
end;
procedure TFrmAd2.ppGroupMonthClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'month');
end;
procedure TFrmAd2.ppGroupYearClick(Sender: TObject);
begin
  GroupDateFormatClick(master, 'year');
end;
procedure TFrmAd2.RelativeToday1Click(Sender: TObject);
begin
  GroupDateFormatClick(master, 'relative');
end;
procedure TFrmAd2.ppFindClick(Sender: TObject);
begin
  FindClick(q_master, Master, Tablename[strtoint(txtid.Text)]);
end;
procedure TFrmAd2.ppShowALLClick(Sender: TObject);
begin
...
//isnowedit master
procedure TFrmAd2.Q_MASTERNewRecord(DataSet: TDataSet);
begin
  NewRecordQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmAd2.Q_MASTERBeforeEdit(DataSet: TDataSet);
begin
  BeforeEditQ(dataset.Tag, Dataset.Name);
end;
procedure TFrmAd2.Q_MASTERBeforeDelete(DataSet: TDataSet);
begin
  BeforeDeleteQ(Dataset.Tag, dataset.Name);
end;
procedure TFrmAd2.Q_MASTERAfterDelete(DataSet: TDataSet);
begin
  AfterDeleteQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmAd2.Q_MASTERBeforePost(DataSet: TDataSet);
begin
  BeforePostQ(DataSet.Tag, Dataset.Name);
end;
procedure TFrmAd2.Q_MASTERAfterPost(DataSet: TDataSet);
begin
  AfterPostQ(DataSet.Tag, dataset.Name);
end;
procedure TFrmAd2.Q_MASTERAfterRefresh(DataSet: TDataSet);
begin
  AutoRefreshQuery;
end;
procedure TFrmAd2.Q_MASTERAfterCancel(DataSet: TDataSet);
begin
...
//procedure query
procedure TFrmAd2.AfterCancelQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
...
procedure TFrmAd2.AfterDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmAd2.AfterPostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  isnowedit[idm] := False;
...
procedure TFrmAd2.ApplicationEvents1Exception(Sender: TObject; E: Exception);
begin
  MsgError(e.Message);
  isNowEdit[strtoint(txtid.Text)]    := False;
end;

procedure TFrmAd2.BeforeDeleteQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
  if nama='master'  then beforeDeleteMaster;
end;
procedure TFrmAd2.BeforeEditQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
procedure TFrmAd2.BeforePostQ(idm : integer; nama:string);
begin
  nama := lowercase(nama);
  nama := replace(nama, 'q_', '');
...
procedure TFrmAd2.NewRecordQ(idm : integer; nama:string);
var
  md : string;
begin
  nama := lowercase(nama);
...
//master
procedure TFrmAd2.newRecordMaster;
begin
  AutoRefreshQuery;
  q_master.setField('tdate', serverdate);
  q_master.setField('amount', '0');
end;
procedure TFrmAd2.beforePostMaster;
begin
...
procedure TFrmAd2.afterPostMaster;
begin
  q_master.Refresh;
end;
procedure TFrmAd2.beforeEditMaster;
begin
  //
end;
procedure TFrmAd2.beforeDeleteMaster;
begin
  if payrollExist(q_master.getFieldString('employee_id'), q_master.getFieldDateTime('tdate')) then
  begin
...
procedure TFrmAd2.afterDeleteMaster;
begin
  //
end;
procedure TFrmAd2.afterCancelMaster;
begin
  //
end;
//end master

procedure TFrmAd2.MASTERemployee_idPropertiesPopup(Sender: TObject);
begin
  LookupADEmployee_id;
end;
procedure TFrmAd2.LookupADEmployee_id;
var
  Q : TZQuery;
...
procedure TFrmAd2.N2ImportALL1Click(Sender: TObject);
begin
  //
